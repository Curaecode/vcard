...
class crons extends CI_Controller {
	...
	function updateunique(){
		$results = $this->db->query("select * from contacts where ssn = account_code  order by id ASC LIMIT 0,100")->result();
		if(!empty($results)){
	...
	function updateimages(){
		...
		fwrite($myfile, $txt);
		$results = $this->db->query("select * from contacts where qrimage is null OR qrimage=''  order by id DESC LIMIT 0,4")->result();
		...
		if(!empty($results)){
			foreach($results as $row){
				...
				if($last_id > 0){
					...
					$this->db->query("update `contacts` set  account_code= '".$account_id."' where id='".$last_id."'");
