    ...
    */
    ...
    /* if ($this->data['url']) { $this->card .= "URL;TYPE=work:".$this->data['url']."\r\n"; } */
	 if ($this->data['url']) { $this->card .= "URL;TYPE=CuraeChoiceCard:".$this->data['url']."\r\n"; } 
	   ...
	   $this->card .= "URL;TYPE=Urology:https://tinyurl.com/choice-urology\r\n";
      ...
      if ($this->data['photo']) { $this->card .= "PHOTO;ENCODING=b;TYPE=JPEG:".$this->data['photo']."\r\n"; }
    ...
    function downloadfile($id=null) 
    {
        $this->filename='';
        $this->getvcard();
        $vcards_hander = fopen('vcards/'.$this->filename.'_'.$id.'.vcf','w');
        fwrite($vcards_hander,$this->card);
        fclose($vcards_hander);
        return true;
    }
